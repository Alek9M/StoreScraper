...
export class Iceland {
    ...
    get nextPagePostfix() {
        ...
        if (this._pageCounter <= 0)
            return "";
        if (this._pageCounter > this._lastPage)
            return undefined;
    ...
    jumpSection() {
        const index = Iceland.foodSections.findIndex((section) => section.postfix === this.currentSection);
    ...
    static async processProductTile(tile) {
        ...
        return {
            ...
            title: title,
            href: href,
            price: new Price(bcp ? currencyTextToNumber(bcp) : price, mm && Number(mm[0]), mm && currencyTextToNumber(mm[1])),
        };
    }
    static async processNutritional(page) {
        ...
        const data = await page.$eval(".table.table-striped.table-sm", (table) => {
            ...
            let index = headers.findIndex((header) => header.textContent?.includes("100g"));
            if (index === -1) {
    ...
    static async processProduct(page, product) {
        ...
        const ingridientsRawElement = await await page.$(".product-right-col-inner");
        let ingridientsRaw;
        try {
            if (ingridientsRawElement && ingridientsRawElement.$(".text-muted")) {
                ingridientsRaw = await ingridientsRawElement.$eval(".text-muted", getText);
            }
        }
        catch (error) {
            console.log(error);
